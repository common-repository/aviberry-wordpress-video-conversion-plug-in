...
# require_once('ApiServer.class.php');
# date_default_timezone_set("UTC");
# $server = ApiServer::getInstance(); //note that the ApiServer class is a singleton
...
class ApiServer {
	...
	protected $requestID;
	protected $requestType;   // XML | JSON
	protected $responseType;  // XML | JSON | JAVASCRIPT
	protected $requestMethod; // GET | POST
	protected $visibility; // PRIVATE | PUBLIC
	...
	public static function getInstance($protocol, $visibility, $debug_mode = false) {
		if (!self::$instance) {
			$c = __CLASS__;
			self::$instance = new $c($protocol);
		}
		
		self::$instance->setVisibility($visibility);
		self::$instance->setDebugMode($debug_mode);
	...
	private function __construct($protocol) {
		...
		$this->requestType   = $protocol;
		$this->responseType  = $protocol;
		
		switch($_SERVER['REQUEST_METHOD']) {
			case 'GET':  $this->requestMethod = self::GET;  break;
			case 'POST': $this->requestMethod = self::POST; break;
			default:     throw new ProtocolError_ApiException();
		}
	...
	**END PHP4*/
	...
	private function init(){
		...
		**END PHP4*/
		...
		#Set error handler
		//
		// АПИ сервер должен реагировать только на ексцепшены. Иначе он шлет на 
		// каждый варнинг сообщение, что не есть правильно. Поэтому комментим
		// следующую строку.
		//
		...
		if(isset($_GET['JSON-response-callback'])){
			...
			if(!preg_match('/^(?:\w|\$|_)(?:\d|\w|\$|_)*(\.(?:\w|\$|_)(?:\d|\w|\$|_)*)*$/', $_GET['JSON-response-callback']))
				throw new Exception("The provided JavaScript callback function \"" . $_GET['JSON-response-callback'] . "\" is not a valid JavaScript identifier.");
	...
	function setDefaultResponseType($responseType){
		switch($responseType){
			...
			#case self::JAVASCRIPT:
				...
				break;
			default:
				throw new Exception('Invalid response type "' . $responseType . '"; it must be one of the constants defined for the ApiServer class: XML or JSON.');
		}
	}
	...
	function setJSONDateFormat($formatName){
		switch($formatName){
			...
			case '@timestamp@':
				...
				break;
			default:
				throw new Exception('Invalid format name "' . $formatName . '" provided for specifying the JSON date format. Format must be either "ISO8601", "classHinting", "@ticks@", or "ASP.NET".');
		}
	...
	function setDBResultIndexType($indexType){
		switch($indexType){
			...
			case 'NUM':
				...
				break;
			default:
				throw new Exception('Invalid index type "' . $indexType . '" provided for specifying database result indices; it must be either "ASSOC" or "NUM".');
		}
	...
	function setVisibility($visibility) {
		switch ($visibility) {
			case self::PUBLIC_VISIBILITY:
			case self::PRIVATE_VISIBILITY:
				$this->visibility = $visibility;
				
				break;
				
			default:
				throw new Exception(sprintf('Unknown visibility "%s".', $visibility));
		}
	}
	...
	function addMethod($privateProcName, $publicMethodName = null){
		...
		if(!function_exists($privateProcName))
			throw new Exception("\$ApiServerInstance->addMethod() failed because the function \"$privateProcName\" does not exist.");
		if(isset($this->publicToPrivateMap[$publicMethodName]))
			throw new Exception("\$ApiServerInstance->addMethod() failed because the method name \"$publicMethodName\" has already been assigned to the function \"" . $this->publicToPrivateMap[$publicMethodName] . "\".");
	...
	public function handleError($errno, $errstr, $errfile, $errline, $errcontext){
		...
		$this->respondWithFault('InternalError_ApiException', $errno, $errstr, $errfile, $errline, $errcontext);
		
		return true;
	}
	
	public function handleException($e){
		$this->isDebugMode && $this->logger->logDebug(__METHOD__ . ": Step Into:\ne=%s", $e);
		
		// Call old handler.
		if (is_callable($this->oldExceptionHandler)) {
			$this->isDebugMode && $this->logger->logDebug(__METHOD__ . ": About to call old exception handler: %s.", $this->oldExceptionHandler);
			call_user_func($this->oldExceptionHandler, $e);
			$this->isDebugMode && $this->logger->logDebug(__METHOD__ . ": Old exception handler was called.");
		}
		
		// Запоминаем данные оригинального исключения.
		$errfile = $e->getFile(); 
		$errline = $e->getLine(); 
		
		// Если это неизвестное нам исключение (т.е. обычный Exception), 
		// то трактуем его как internal error и именно так оно бкдет преподнесено
		// конечному пользователю API (см. рздел "Использование исключений" в 
		// описнии архитектуры API). 
		if (!is_subclass_of($e, 'ApiException')) {
			$e = new InternalError_ApiException(false, false, $e->getMessage());
		}
		
		// Код и сообщение отображаем текущего сообщения, которое может быть
		// преобразованным стандартным Exception-ом. Но файл и строку, где 
		// произошла ошибка, показываем оригинальные, т.е. фактическое место 
		// ошибки.
		
		$errcode = $e->getCode();
		$errmess = $e->getMessage();
		
		$this->isDebugMode && $this->logger->logDebug(__METHOD__ . ": About to send respond with fault:\ncode=%s,\nmess=%s,\nfile=%s,\nline=%s", $errcode, $errmess, $errfile, $errline);
		$this->respondWithFault(get_class($e), $errcode, $errmess, $errfile, $errline, $e->getTrace());
		$this->isDebugMode && $this->logger->logDebug(__METHOD__ . ": Respond with fault was sent.");
		
		$this->isDebugMode && $this->logger->logDebug(__METHOD__ . ": Step Out: %s", true);
		return true;
	}
	
	/**
	 * Called on shutdown to catch fatal errors and make the correct RPC respond to the user.
	 */
	public function onShutDown() {
		$error = error_get_last();
		$this->isDebugMode && $this->logger->logDebug('$error: ' . var_export($error, true));
		...
		if(!empty($error) &&
			...
			$this->isDebugMode && $this->logger->handleError($error['type'], $error['message'], $error['file'], $error['line']);
			
			// Передаем дополнительную информацию в самом сообщении об ошибке,
			// для того чтобы мы сразу видели положение дел без дополнителных 
			// "поползновений".
			if ($this->isDebugMode)
				$e = new InternalError_ApiException('', 0, $error['message']);
			else			
				$e = new InternalError_ApiException();
			
			$this->respondWithFault(get_class($e), $e->getCode(), $e->getMessage(), $error['file'], $error['line']);
		}
	}

	public function respondWithFault($errclass, $errno, $errstr, $errfile = null, $errline = null, $errdetails = null){
		ob_clean();
		...
		if($this->responseType == self::XML){
			...
			print '<member>';
				print '<name>faultClass</name>';
				print '<value><string>' . htmlspecialchars($errclass, ENT_NOQUOTES) . '</string></value>';
			print '</member>';
			print '<member>';
			...
			print '<member>';
				...
				print '<value><string>' . htmlspecialchars($errstr, ENT_NOQUOTES) . '</string></value>';
			print '</member>';
			...
			//
			// XML-RPC более старый и проработанный стандарт, да и наверно, написан
			// более умными людьми, ну или более опытными, если кому угодно. Поэтому
			// следующие строки закоменчены по аналогии с XML-RPC, иначе конечный 
			// пользователь JSON-RPC может никогда и не увидеть сообщения ему посланные.
			//
			...
			*/
			...
			print '"class":' . $this->encodeJson($errclass) . ',';
	...
	function dieWithStatus($status) {
		...
		if (!in_array($protocol, array('HTTP/1.0', 'HTTP/1.1')))
			$protocol = 'HTTP/1.0';
				
		$status_header = "$protocol $status $status_message";
		header($status_header, true, $status);
		die($status_message);
	}
	
	function processRequest(){
		$this->isDebugMode && $this->logger->logDebug("processRequest(): Step Into:\nthis=%s,\nSERVER=%s", $this, $_SERVER);
		...
		if ($this->requestMethod == self::GET) {
			$this->isDebugMode && $this->logger->logDebug("processRequest(): Request method is GET: %s", $_SERVER['REQUEST_URI']);
			
			$url_parts = parse_url($_SERVER['REQUEST_URI']);
			$this->isDebugMode && $this->logger->logDebug("processRequest(): Parse URL: %s", $url_parts);

			// For support of internal code, where accuracy of error messages
			// matters not so, and REWRITE_BASE is not defined.
			if (!defined('REWRITE_BASE'))
				define('REWRITE_BASE', '~~'); // Empty regex.
				
			$count = 0;
			$path = preg_replace(REWRITE_BASE, '', $url_parts['path'], 1, $count);
			$this->publicMethodName = basename($path);
			
			// If there is no replacements, then wrong URI was specified.
			if (!$count)
				$this->dieWithStatus('404');
				
			if (empty($this->publicMethodName)) {
				switch ($this->visibility) {
					case self::PUBLIC_VISIBILITY:
						throw new InvalidRequest_ApiException(false, false, 'Method is not specified.');
			...
			$requestParams = $this->decodeQuery(isset($url_parts['query']) ? $url_parts['query'] : '');
			$this->isDebugMode && $this->logger->logDebug("processRequest(): Decode query: %s", $requestParams);
		} else { 
			if ($this->requestType == self::XML) {
				...
				if(!($doc = domxml_open_mem($this->requestData, DOMXML_LOAD_PARSING, &$error)))
					throw new Exception("Parse error: " . join(" ... ", $error));
				**END PHP4*/
				
				#The payload is in XML, a single <methodCall> structure.
				if(($root = $doc->documentElement) && $root->nodeName != 'methodCall')
					throw new Exception("The root of the document must be a 'methodCall' element.");
				unset($root);
				   ...
				   $methodNameElements->item(0)->firstChild->nodeValue)
				{
					...
					if(preg_match("/[^A-Z0-9_\.:\/]/i", $this->publicMethodName))
						throw new Exception("The supplied method name \"" . $this->publicMethodName . "\" contains illegal characters. It may may only contain identifier characters, upper and lower-case A-Z, the numeric characters, 0-9, underscore, dot, colon and slash.");
				}
				...
				for($i = 0; $i < $paramElements->length; $i++){
					...
					if(!$valueEl)
						throw new Exception("XML-RPC Parse Error: Expected a 'value' element child of the 'param' element.");
					array_push($requestParams, $this->decodeXmlRpc($valueEl));
				}
				
			} else if($this->requestType == self::JSON) {
				if (!$this->requestData)
					throw new InvalidRequest_ApiException();
				...
				$this->isDebugMode && $this->logger->logDebug("processRequest(): Request: %s", $request);
				
				if(!$request)
					throw new Exception("Parse error in JSON input.");
				if(!is_array($request))
					throw new Exception("Invalid format for JSON-RPC request.");
				...
				if(!isset($request['version']))
					throw new Exception("The JSON request object must provide the JSON-RPC version.");
				if((float) $request['version'] > 1.1)
					throw new Exception("This JSON-RPC library supports version 1.1, but you are attempting to use version " . $request['version'] . ".");
				 */
				...
				if(strrpos($this->publicMethodName, '/') == strlen($this->publicMethodName)-1)
					throw new Exception("The procedure's name must not end in a forward-slash.");
			...
			else throw new Exception("Unable to discern the requested method name. You must pass the request as "  .
		...
		if (!$this->publicMethodName)
			throw new MethodNotFound_ApiException();
		...
		**END PHP4*/
			...
			foreach(array_keys($this->publicToPrivateMap) as $publicProc){
				...
				foreach($rf->getParameters() as $param){ #$i => 
					if($param->isPassedByReference())
						throw new Exception("User functions cannot be defined with parameters passed by reference. The function \"$functionName\" wants the parameter \"" . $param->getName() . "\" to be passed by reference.");
					...
					if($param->isArray())
						...
						switch(strtolower($rc->getName())){
							case 'datetime':
								...
								break;
							default:
								throw new Exception("The only class type that may be hinted is DateTime; you provided \"" . $rc->getName() . "\" in the function \"$functionName\".");
						}
					}
		...
		#Using the PHP tokenizer before PHP 5.1.0
							...
							if(is_array($tokens[$i])){
								...
								if($parenDepth == 1 && $tokens[$i][0] == T_VARIABLE){
										...
										if($tokens[$i-1][0] == T_STRING){
											switch(strtolower($tokens[$i-1][1])){
												case 'datetime':
													...
													break;
												default:
													throw new Exception("The only class type that may be hinted is DateTime; you provided \"" . $tokens[$i-1][1] . "\" in the function \"$functionName\".");
											}
										}
										#Array parameter
									...
									#Of course data may not be passed from the client by reference
										...
										throw new Exception("User functions cannot be defined with parameters passed by reference. The function \"$functionName\" wants the parameter \"" . $paramDetails['name'] . "\" to be passed by reference.");
			...
			foreach($this->publicToPrivateMap as $publicProc => $privateProc){
				if(!isset($parametersForPrivateProcs[$privateProc]))
					throw new Exception("Because this version of PHP does not support the Reflection API, unable to use the public method \"$publicProc\" because its associated private ".
										"procedure is located in an included file; to get around this, you must explicitly ".
										"allow externally defined functions by calling \$ApiServerInstance->useIncludedFunctions(true); this will decrease performance. ".
		...
		//	throw new Exception('This implementation does not support "system.methodSignature" since PHP does not have type declarations.');
		//	
		...
		//		throw new Exception('The "system.methodSignature" method must be supplied a method name.');
		//	if(!isset($this->publicToPrivateMap[$requestParams[0]]))
		//		throw new Exception('The supplied method name "' . $requestParams[0] . '" does not exist.');
		//}
		...
		//	throw new Exception('This implementation does not support "system.methodHelp" since PHP does not have type declarations.');
		//}
		...
		//	throw new Exception('This implementation does not support "system.describeMethods" since PHP does not have type declarations.');
		//}
		else if($this->publicMethodName == 'system.setJSONDateFormat'){
			if(!count($requestParams))
				throw new Exception('Parameter must be either "ISO8601", "classHinting", "@ticks@", or "ASP.NET".');
			...
			if(!count($requestParams))
				throw new Exception('Parameter must be either "NUM" or "ASSOC"..');
			$keys = array_keys($requestParams);
			...
			if (
				   !isset($this->publicToPrivateMap[$this->publicMethodName]) 
				|| !function_exists($this->publicToPrivateMap[$this->publicMethodName])
			) {
				$this->isMethodNotFound = true;
				throw new MethodNotFound_ApiException(false, false, sprintf('Method: %s', $this->publicMethodName));
			}
			...
			if(!is_array($requestParams))
				throw new Exception("The parameter(s) supplied must be in the form of a positional array or an associative array (JSON Object).");
		...
		$this->isDebugMode && $this->logger->logDebug("processRequest(): Step Out");
	} 
	...
	protected function encodeJson($value){
		...
		//case 'string':
			//Note: in PHP 5.1.2, using 'ApiServer' for &$this raises strict error: Non-static method cannot not be called statically, even though it is declared statically.
			$value = preg_replace_callback('/([\\\\\/"\x00-\x1F])/', array(&$this, 'getEscapeSequence_callback'), $value); 
			return '"' . /*utf8_encode*/($value) . '"';
		}
		throw new Exception("Unable to convert type " . gettype($value) . " to JSON.");
	}

	## Converts HTTP GET params to PHP ##########################
	protected function decodeQuery($query) {
		$result = array();
		
		$params = array_diff(explode('&', $query), array(''));
		foreach ($params as $param) {
			$param = array_diff(explode('=', $param), array(''));
			if (count($param) != 2)
				throw new ParseError_ApiException();
				
			$matches = array();
			if (preg_match('/([_a-z][_a-z0-9]*)((?:\[(?:[_a-z0-9][_a-z0-9]*)?\])+)/i', $param[0], $matches)) {
				$param_name = $matches[1];
				$param_keys = $matches[2];
				if (!array_key_exists($param_name, $result))
					$result[$param_name] = array();
					
				$matches = array();
				preg_match_all('/(?:\[([_a-z0-9][_a-z0-9]*)?\])/i', $param_keys, $matches);
				
				$subobj = &$result[$param_name];
				for ($i = 0; $i < count($matches[1]); $i++) {
					$key = $matches[1][$i];
					
					if ($i == (count($matches[1]) - 1)) // If last element.
						// On the bottom. Write value.
						if ($key == '')
							$subobj[]     = rawurldecode($param[1]);
						else
							$subobj[$key] = rawurldecode($param[1]);
					else {
						// Fall into the deep. Create keys.
						if ($key == '') {
							$subobj[] = array();
							$subobj   = &$subobj[count($subobj) - 1];
						} else {
							if (array_key_exists($key, $subobj)) {
								$subobj = &$subobj[$key];
							} else {
								$subobj[$key] = array();
								$subobj       = &$subobj[$key];
	...
	protected function decodeJson_processTokens(&$tokens, &$i){
			...
			default:
				...
				if($c == '"'){
					...
					#Regular string
						...
						//Note: in PHP 5.1.2, using 'ApiServer' for &$this raises strict error: Non-static method cannot not be called statically, even though it is declared statically (also below)
		...
		throw new Exception("Parse error in JSON input. Error detected at token " . ($i+1) . ": " . (isset($tokens[$i]) ? join('', array_slice($tokens, $i)) : 'EOF'));
	}
	...
	protected function encodeXmlRpc($value){
		...
		//default:
			...
			throw new Exception("Unknown PHP data type: " . gettype($value));
		$xml .= '</value>';
		return $xml;
	}
				...
				switch($typeEl->nodeName){
					...
					case 'int':
						...
						if(!preg_match("/^[-\+]?\d+$/", $typeEl->firstChild->nodeValue))
							throw new Exception("XML-RPC Parse Error: The value provided as an integer '" . $typeEl->firstChild->nodeValue . "' is invalid.");
					...
					case 'double':
						...
						if(!preg_match("/^[-\+]?\d+(\.\d+)?$/", $typeEl->firstChild->nodeValue))
							throw new Exception("XML-RPC Parse Error: The value provided as a double '" . $typeEl->firstChild->nodeValue . "' is invalid.");
						return (double) $typeEl->firstChild->nodeValue;
					case 'boolean':
						if($typeEl->firstChild->nodeValue != '0' && $typeEl->firstChild->nodeValue != '1')
							throw new Exception("XML-RPC Parse Error: The value provided as a boolean '" . $typeEl->firstChild->nodeValue . "' is invalid.");
					...
					case 'dateTime.iso8601':
						...
						#}
						#catch(Exception $e){
						#	throw new Exception("XML-RPC Parse Error: The value provided as a dateTime.iso8601 '" . $typeEl->firstChild->nodeValue . "' is invalid.");
						#}	
						return $date;
					case 'base64':
					...
					case 'array':
						...
						if(!$dataEl)
							throw new Exception("XML-RPC Parse Error: Expected 'data' element as sole child element of 'array'.");
						...
						while($valueEl){
							if($valueEl->nodeType == 1){
								...
								if($valueEl->nodeName == 'value')
									...
									throw new Exception("XML-RPC Parse Error: Illegal element child '" . $valueEl->nodeName . "' of an array's 'data' element.");
							}
							$valueEl = $valueEl->nextSibling;
						}
						...
						return $arr;
					default:
						throw new Exception("XML-RPC Parse Error: Illegal element '" . $typeEl->nodeName . "' child of the 'value' element.");
				}
			}
		}
		return '';
	...
	protected function convertResource($resource){
		...
		switch($resourceType){
			...
			default:
				throw new Exception("Unable to return resource type '$resourceType'.");
		}
	}
