...
rpc.ServiceProxy = function(serviceURL, options) {
	...
	var urlParts = this.__serviceURL
			.match(/^(\w+:)\/\/(?:\w+:\w+@)?([^\/]+?)(?::(\d+))?(?:$|\/)/);
	if (urlParts) {
		this.__isCrossSite = (location.protocol != urlParts[1]
				|| document.domain != urlParts[2] || location.port != (urlParts[3] || ""));
	}
	...
	this.__dateEncoding = 'ISO8601'; // ("@timestamp@" || "@ticks@") ||
										// "classHinting" || "ASP.NET"
	...
	if (options instanceof Object) {
		if (options.asynchronous !== undefined) {
			...
			if (!this.__use_flXHR && !this.__isAsynchronous
					&& this.__isCrossSite)
		...
		if (options.use_flXHR != undefined) {
			...
			this.__flproxy = null;
			this.__crossdomain_xml = options.crossdomain_xml;
	...
	// Create local "wrapper" functions which reference the methods obtained
	// above
	for ( var methodName, i = 0; methodName = this.__methodList[i]; i++) {
		// Make available the received methods in the form of chained property
		// lists (eg. "parent.child.methodName")
		...
		// Create a wrapper to this.__callMethod with this instance and this
		// methodName bound
		var wrapper = (function(instance, methodName) {
			var call = {
				instance : instance,
				methodName : methodName
			}; // Pass parameters into closure
			return function() {
				if (call.instance.__isAsynchronous) {
					if (arguments.length == 1 && arguments[0] instanceof Object) {
						call.instance.__callMethod(call.methodName,
								arguments[0].params, arguments[0].onSuccess,
					...
					return call.instance.__callMethod(call.methodName, rpc
							.toArray(arguments));
			};
		})(this, methodName);
...
rpc.ServiceProxy.prototype.__callMethod = function(methodName, params,
		successHandler, exceptionHandler, completeHandler, local_params) {
	rpc.requestCount++;

	// Verify that successHandler, exceptionHandler, and completeHandler are
	// functions
	if (this.__isAsynchronous) {
		if (successHandler && typeof successHandler != 'function')
			throw Error('The asynchronous onSuccess handler callback function you provided is invalid; the value you provided ('
					+ successHandler.toString()
					+ ') is of type "'
					+ typeof (successHandler) + '".');
		if (exceptionHandler && typeof exceptionHandler != 'function')
			throw Error('The asynchronous onException handler callback function you provided is invalid; the value you provided ('
					+ exceptionHandler.toString()
					+ ') is of type "'
					+ typeof (exceptionHandler) + '".');
		if (completeHandler && typeof completeHandler != 'function')
			throw Error('The asynchronous onComplete handler callback function you provided is invalid; the value you provided ('
					+ completeHandler.toString()
					+ ') is of type "'
	...
	try {
		...
		if (this.__isCrossSite) { // then this.__isAsynchronous is implied
			...
			// Create an ad hoc function specifically for this cross-site
			// request; this is necessary because it is
			// not possible pass an JSON-RPC request object with an id over HTTP
			// Get requests.
			rpc.callbacks['r' + String(rpc.requestCount)] = (function(instance,
					id) {
				var call = {
					instance : instance,
					id : id
				}; // Pass parameter into closure
			...
			// rpc.callbacks['r' + String(rpc.requestCount)] = new
			// Function("response", 'response.id = ' + rpc.requestCount + ';
			// this.__doCallback(response);');

			// Make the request by adding a SCRIPT element to the page
			var script = document.createElement('script');
			script.setAttribute('type', 'text/javascript');
			var src = this.__serviceURL + '/' + methodName + '?'
					+ this.__callbackParamName + '=rpc.callbacks.r'
					+ (rpc.requestCount);
			if (params)
		...
		// ------------------------------------------------------------
			...
			if (params) {
				if (!(params instanceof Object) || params instanceof Date) // JSON-RPC
																			// 1.1
																			// allows
																			// params
																			// to
																			...
																			// an
																			// array
					throw Error('When making asynchronous calls, the parameters for the method must be passed as an array (or a hash); the value you supplied ('
							+ String(params)
							+ ') is of type "'
							+ typeof (params) + '".');
			...
			if (this.__protocol == 'XML-RPC') {
				...
				if (params) {
					...
					for ( var i = 0; i < params.length; i++)
						xml
								.push('<param>' + this.__toXMLRPC(params[i]) + '</param>');
					xml.push('</params>');
				}
				xml.push('</methodCall>');
				postData = xml.join('');
			...
			// XMLHttpRequest chosen (over Ajax.Request) because it propogates
			// uncaught exceptions
			var xhr;
			...
			if (!this.__use_flXHR) {
				...
				if (this.__flproxy == null) {
					this.__flproxy = new flensed.flXHR( {
						autoUpdatePlayer : true,
						noCacheHeader : false,
						xmlResponseText : false,
						loadPolicyURL : this.__crossdomain_xml
					});					
				}
			...
			var url = this.__serviceURL;
			if(this.httpMethod == 'GET'){
				url += methodName + '/';
			}
	
			xhr.open(this.httpMethod, url, this.__isAsynchronous,
					this.__authUsername, this.__authPassword);
			if (this.__authUsername != undefined){
				// IE do not known about btoa. Using escape() in IE
				if (!window.btoa) 
					auth = escape(this.__authUsername + ':' + this.__authPassword);
				else
					auth = window.btoa(this.__authUsername + ':' + this.__authPassword);
			...
			if (this.__isAsynchronous) {
				...
				var requestInfo = {
					id : rpc.requestCount
				}; // for XML-RPC since the 'request' object cannot contain
					// request ID
				xhr.onreadystatechange = function() {
					...
					if (xhr.readyState == 4) {
						...
						if (instance.__protocol == 'XML-RPC') {
							var response = instance.__getXMLRPCResponse(xhr,
									requestInfo.id);
							instance.__doCallback(response);
						}
						// JSON-RPC
							...
							var response = instance.__evalJSON(
									xhr.responseText,
									instance.__isResponseSanitized);
							if (!response.id)
			...
			// -----------------------------------------------------
				...
				if (this.__protocol == 'XML-RPC')
					...
					response = this.__evalJSON(xhr.responseText,
							this.__isResponseSanitized);

				// Note that this error must be caught with a try/catch block
				// instead of by passing a onException callback
				if (response.error){
					var message = response.error.message ? 
						response.error.message :
						'Can not to call "' + methodName
...
// This acts as a lookup table for the response callback to execute the
// user-defined
// callbacks and to clean up after a request
rpc.pendingRequests = {};

// Ad hoc cross-site callback functions keyed by request ID; when a cross-site
// request
...
rpc.ServiceProxy.prototype.__doCallback = function(response) {
	...
	// Remove the SCRIPT element from the DOM tree for cross-site
	// (JSON-in-Script) requests
		...
		if (rpc.pendingRequests[response.id].onException) {
			try {
				if (!rpc.pendingRequests[response.id].onException(err,
						rpc.pendingRequests[response.id]._local_params))
	...
	// Process the valid result
		...
		// abstraction allows for accessing all of the results members by
		// reference.
		this.__upgradeValuesFromJSON(response);
		...
		if (rpc.pendingRequests[response.id].onSuccess) {
			...
			// If the onSuccess callback itself fails, then call the onException
			// handler as above
			catch (err) {
	...
	// Merge any exception raised by onComplete into the previous one(s) and
	// throw it
	if (uncaughtExceptions.length) {
		var code;
		var message = 'There ' + (uncaughtExceptions.length == 1 ? 'was 1 uncaught exception'
...
rpc.ServiceProxy.prototype.__toJSON = function(value) {
	...
	case 'object':
		if (value === null)
			...
			var json = [ '[' ]; // Ext's JSON.js reminds me that Array.join is
								// faster than += in MSIE
			...
			case 'classHinting': // {"__jsonclass__":["constructor",
									// [param1,...]], "prop1": ...}
			...
			var useHasOwn = {}.hasOwnProperty ? true : false; // From Ext's
																// JSON.js
			var json = [ '{' ];
			for ( var key in value) {
				if (!useHasOwn || value.hasOwnProperty(key)) {
					if (json.length > 1)
						json.push(',');
					json.push(this.__toJSON(key) + ':'
							+ this.__toJSON(value[key]));
				}
			}
			json.push('}');
			return json.join('');
	...
	throw new TypeError(
			'Unable to convert the value of type "' + typeof (value) + '" to JSON.'); // (' +
																						// String(value)
																						// + ')
};
...
rpc.ServiceProxy.prototype.__evalJSON = function(json, sanitize) { // from
																	// Prototype
																	// String.evalJSON()
// alert(json);
	if (json == null)
...
// This function iterates over the properties of the passed object and converts
// them
// into more appropriate data types, i.e. ISO8601 strings are converted to Date
// objects.
	...
	for ( var key in obj) {
		if (!useHasOwn || obj.hasOwnProperty(key)) {
			...
			if (typeof obj[key] == 'string') {
				// ISO8601
				if (this.__decodeISO8601
						&& (matches = obj[key]
								.match(/^(?:(\d\d\d\d)-(\d\d)(?:-(\d\d)(?:T(\d\d)(?::(\d\d)(?::(\d\d)(?:\.(\d+))?)?)?)?)?)$/))) {
					obj[key] = new Date(0);
					if (matches[1])
				...
				// JSON 1.0 Class Hinting: {"__jsonclass__":["constructor",
				// [param1,...]], "prop1": ...}
				if (obj[key].__jsonclass__ instanceof Array) {
					// console.info('good1');
					if (obj[key].__jsonclass__[0] == 'Date') {
						// console.info('good2');
						if (obj[key].__jsonclass__[1] instanceof Array
								&& obj[key].__jsonclass__[1][0])
...
rpc.ServiceProxy.prototype.__toXMLRPC = function(value) {
	...
	case 'object':
		if (value === null)
			...
			xml
					.push('<dateTime.iso8601>' + rpc.dateToISO8601(value) + '</dateTime.iso8601>');
		} else if (value instanceof Number || value instanceof String
				|| value instanceof Boolean)
	...
	default:
		throw new TypeError(
				'Unable to convert the value of type "' + typeof (value) + '" to XML-RPC.'); // (' +
																								// String(value)
																								// + ')
	}
	xml.push('</value>');
...
// //}
// else {
// var numEl = document.createElement(parseInt(value) == Math.ceil(value) ?
// 'int' : 'double');
...
// }
// else if(value instanceof Number || value instanceof String || value
// instanceof Boolean)
// return rpc.dateToISO8601(value.valueOf());
// else {
...
// //default:
// }
// throw new TypeError('Unable to convert the value of type "' + typeof(value) +
// '" to XML-RPC.'); //(' + String(value) + ')
// };
...
rpc.ServiceProxy.prototype.__parseXMLRPC = function(valueEl) {
	...
	for ( var i = 0; i < valueEl.childNodes.length; i++) {
		if (valueEl.childNodes.item(i).nodeType == 1) {
			...
			case 'i4':
			case 'int':
				// An integer is a 32-bit signed number. You can include a plus
				// or minus at the
				// beginning of a string of numeric characters. Leading zeros
				// are collapsed.
				// Whitespace is not permitted. Just numeric characters
				// preceeded by a plus or minus.
				var intVal = parseInt(typeEL.firstChild.nodeValue);
				if (isNaN(intVal))
					throw Error("XML-RPC Parse Error: The value provided as an integer '"
							+ typeEL.firstChild.nodeValue + "' is invalid.");
				return intVal;
			case 'double':
				// There is no representation for infinity or negative infinity
				// or "not a number".
				// At this time, only decimal point notation is allowed, a plus
				// or a minus,
				// followed by any number of numeric characters, followed by a
				// period and any
				// number of numeric characters. Whitespace is not allowed. The
				// range of
				// allowable values is implementation-dependent, is not
				// specified.
				var floatVal = parseFloat(typeEL.firstChild.nodeValue);
				if (isNaN(floatVal))
					throw Error("XML-RPC Parse Error: The value provided as a double '"
							+ typeEL.firstChild.nodeValue + "' is invalid.");
				return floatVal;
			case 'boolean':
				if (typeEL.firstChild.nodeValue != '0'
						&& typeEL.firstChild.nodeValue != '1')
					throw Error("XML-RPC Parse Error: The value provided as a boolean '"
							+ typeEL.firstChild.nodeValue + "' is invalid.");
			...
			case 'datetime.iso8601':
				...
				if (matches = typeEL.firstChild.nodeValue
						.match(/^(?:(\d\d\d\d)-(\d\d)(?:-(\d\d)(?:T(\d\d)(?::(\d\d)(?::(\d\d)(?:\.(\d+))?)?)?)?)?)$/)) {
					if (matches[1])
						date.setUTCFullYear(parseInt(matches[1]));
					if (matches[2])
						date.setUTCMonth(parseInt(matches[2] - 1));
					if (matches[3])
						date.setUTCDate(parseInt(matches[3]));
					if (matches[4])
						date.setUTCHours(parseInt(matches[4]));
					if (matches[5])
						date.setUTCMinutes(parseInt(matches[5]));
					if (matches[6])
						date.setUTCMilliseconds(parseInt(matches[6]));
					return date;
				}
			...
			case 'nil':
				return null;
			case 'struct':
				// A <struct> contains <member>s and each <member> contains a
				// <name> and a <value>.
				var obj = {};
				for ( var memberEl, j = 0; memberEl = typeEL.childNodes.item(j); j++) {
					if (memberEl.nodeType == 1 && memberEl.nodeName == 'member') {
						var name = '';
						valueEl = null;
						for ( var child, k = 0; child = memberEl.childNodes
								.item(k); k++) {
							if (child.nodeType == 1) {
						...
						// <struct> or
						// any other type, including an <array>, described
						// below.
						if (name && valueEl)
			...
			case 'array':
				...
				while (dataEl
						&& (dataEl.nodeType != 1 || dataEl.nodeName != 'data'))
					dataEl = dataEl.nextSibling;

				if (!dataEl)
					new Error(
							"XML-RPC Parse Error: Expected 'data' element as sole child element of 'array'.");
				...
				while (valueEl) {
					if (valueEl.nodeType == 1) {
						...
						// <array> or
						// any other type, including a <struct>, described
						// above.
						if (valueEl.nodeName == 'value')
							arr.push(this.__parseXMLRPC(valueEl));
						else
							throw Error("XML-RPC Parse Error: Illegal element child '"
									+ valueEl.nodeName
									+ "' of an array's 'data' element.");
					}
					valueEl = valueEl.nextSibling;
				}
				return arr;
			default:
				throw Error("XML-RPC Parse Error: Illegal element '"
						+ typeEL.nodeName + "' child of the 'value' element.");
			}
		}
	}
	return '';
...
// Takes an array or hash and coverts it into a query string, converting dates
// to ISO8601
			...
			if (params[key] instanceof Array) {
				for ( var i = 0; i < params[key].length; i++) {
					...
					if (params[key][i] instanceof Date)
						str += encodeURIComponent(rpc
								.dateToISO8601(params[key][i]));
...
// Converts an iterateable value into an array; similar to Prototype's $A
// function
...
// Returns an ISO8601 string *in UTC* for the provided date (Prototype's
// Date.toJSON() returns localtime)
rpc.dateToISO8601 = function(date) {
	// var jsonDate = date.toJSON();
	// return jsonDate.substring(1, jsonDate.length-1); //strip double quotes

	return date.getUTCFullYear() + '-' + rpc.zeroPad(date.getUTCMonth() + 1)
